...
describe('CutText', () => {
  ...
  it('should return an error if "content" arg length = 0', () => {
    const emptyContent = '';
    expect(cutText(emptyContent, 20)).to.equal('Error');
  });
  ...
  it('should return an error if "maxLengh" is lower or equal 0', () => {
    if (typeof maxLength !== 'number' || maxLength <= 0) return 'Error'; // Dodana walidacja typu i długości
